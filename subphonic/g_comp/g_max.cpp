...
GMax::GMax(BitmapFont* f, GroundMixer& g) : SoundComp(g)
{
...
Value** GMax::getOutput(unsigned int id)
{
    return (Value**)&sig;
}

void GMax::addInput(Value** out, unsigned int fromid)
{
    if(fromid==0)sig->setInput0(out);
    if(fromid==1)sig->setInput1(out);
}

void GMax::remInput(Value** out, unsigned int fromid)
{

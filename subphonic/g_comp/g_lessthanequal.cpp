...
GLessThanEqual::GLessThanEqual(BitmapFont* f, GroundMixer& g) : SoundComp(g)
{
    ...
    sig = new LessThanEqual();
   
}

GLessThanEqual::~GLessThanEqual()
{
    delete sig;
}


Value** GLessThanEqual::getOutput(unsigned int id)
{
    return (Value**)&sig;
}

void GLessThanEqual::addInput(Value** out, unsigned int fromid)
{
    if(fromid==0)sig->setInput0(out);
    if(fromid==1)sig->setInput1(out);
}

void GLessThanEqual::remInput(Value** out, unsigned int fromid)
{
...
/*
  void GLessThanEqual::getState(SoundCompEvent* e)
  {
  }

  void GLessThanEqual::setState(SoundCompEvent* e)
  {
  DASSERT(e!=NULL);
  }
*/

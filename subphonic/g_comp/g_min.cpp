...
GMin::GMin(BitmapFont* f, GroundMixer& g) : SoundComp(g)
{
...
Value** GMin::getOutput(unsigned int id)
{
    return (Value**)&sig;
}

void GMin::addInput(Value** out, unsigned int fromid)
{
    if(fromid==0)sig->setInput0(out);
    if(fromid==1)sig->setInput1(out);
}

void GMin::remInput(Value** out, unsigned int fromid)
{
